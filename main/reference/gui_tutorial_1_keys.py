...
def initialize(timestamp):
    ...
    # initialize() 내용물을 실행하는 동안에는 키 입력을 받아올 수 없어요.
    # 그렇기는 하지만, 나중에 update() 실행 도중 어떤 키(들)을 사용할 것인지를 미리 준비해둘 수 있어요
...
def update(timestamp):
    ...
    while idx_skill < len(w.data.keys_use_skill):
        ...
        if w.keys[key]:
            ...
            # 'break문'을 실행하면, 이 문장을 내용물로 두고 있는 그 while문/for문의 실행을 중단해요.
            # - Code 실행 흐름 측면에서 return문과 유사하게 동작하지만,
        ...
        # -> 조건식 계산 결과값이 False였을 테니, while문의 else부분 내용물을 실행한 다음에는 전체 while문 실행이 끝나요

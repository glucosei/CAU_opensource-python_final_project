...
class Window:
    ...
    def start(self):
        ...
        if sys.getrefcount(self.internals얘는안봐도돼요.tkinter) > 11:
            print('''경고: tkinter 모듈은 굳이 import하지 않아도 돼요.
인터넷에서 코드 가져와서 진행하는 것보다 수업 내용 복습하면서 천천히 직접 적어 보는 것을 추천해요.
- gui_blank.py에 마련해 둔 기본 구조를 준수하지 않는 경우 전체 프로젝트 점수가 0점으로 간주될 수 있어요''')
    ...
    def newRectangle(self, x, y, width, height, fill_color='black', outline_thickness=0, outline_color='', isVisible=True):
        ...
        number = self.internals얘는안봐도돼요.canvas.create_rectangle(x, y, x + width, y + height, fill=fill_color, width=outline_thickness, outline=outline_color, state=self.internals얘는안봐도돼요.tkinter.NORMAL if isVisible else self.internals얘는안봐도돼요.tkinter.HIDDEN)
    ...
    def newOval(self, x, y, width, height, fill_color='black', outline_thickness=0, outline_color='', isVisible=True):
        ...
        number = self.internals얘는안봐도돼요.canvas.create_oval(x, y, x + width, y + height, fill=fill_color, width=outline_thickness, outline=outline_color, state=self.internals얘는안봐도돼요.tkinter.NORMAL if isVisible else self.internals얘는안봐도돼요.tkinter.HIDDEN)
    ...
    def newImage(self, x, y, filename, new_width=None, new_height=None, isVisible=True, isPixelwiseModifiable=False):
        ...
        if filename is None:
            ...
            img = self.internals얘는안봐도돼요.tkinter.PhotoImage(width=new_width, height=new_height)
        elif not filename in self.internals얘는안봐도돼요.imagesFromFiles:
            # ☆ 해당 파일을 처음 사용하는 경우 읽어 와 저장해 둠. 이미 읽은 적이 있는 경우 저장해 둔 것을 재사용
            img = self.internals얘는안봐도돼요.tkinter.PhotoImage(file=filename)
        ...
        if isPixelwiseModifiable:
            ...
            if new_width % org_width == 0 and new_height % org_height == 0:
                ...
                newImg = self.internals얘는안봐도돼요.tkinter.PhotoImage(width=new_width, height=new_height)
            ...
            if not tag_img in self.internals얘는안봐도돼요.images:
                ...
                if new_width == org_width and new_height == org_height:
                    ...
                    try:
                        img = img.zoom(new_width, new_height).subsample(org_width, org_height)
                    except self.internals얘는안봐도돼요.tkinter.TclError:
                        newImg = self.internals얘는안봐도돼요.tkinter.PhotoImage(width=new_width, height=new_height)
                        rate_x = org_width / new_width
                        rate_y = org_height / new_height
                        for y_new in range(new_height):
        ...
        number = self.internals얘는안봐도돼요.canvas.create_image(x, y, anchor=self.internals얘는안봐도돼요.tkinter.NW, image=img, state=self.internals얘는안봐도돼요.tkinter.NORMAL if isVisible else self.internals얘는안봐도돼요.tkinter.HIDDEN)
    ...
    def newText(self, x, y, width, text='', fill_color='black', anchor='center', isVisible=True):
        ...
        number = self.internals얘는안봐도돼요.canvas.create_text(x, y, width=width, text=text, fill=fill_color, anchor=anchor, state=self.internals얘는안봐도돼요.tkinter.NORMAL if isVisible else self.internals얘는안봐도돼요.tkinter.HIDDEN)
    ...
    def setImage(self, number, new_filename, new_width=None, new_height=None):
        ...
        if not new_filename in self.internals얘는안봐도돼요.imagesFromFiles:
            new_img = self.internals얘는안봐도돼요.tkinter.PhotoImage(file=new_filename)
    ...
    def __init__(self, title='개발중!', width=800, height=950, interval=1/60, printKeyInfos=False, printMouseButtonIdxs=False, isDebugMode=False):
        import tkinter
        ...
        class Internals:
            ...
            def __init__(self_internals, width, height, printKeyInfos, printMouseButtonIdxs, isDebugMode):
                self_internals.tkinter = tkinter
                ...
                self_internals.canvas = tkinter.Canvas(self_internals.frame, width=width, height=height, highlightthickness=0, bg='skyblue')
